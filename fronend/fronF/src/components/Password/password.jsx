...
function Password() {
  ...
  useEffect(() => {
    ...
    if (!uid || !tk) {
      setMessage({ type: "danger", text: "❌ Error: URL inválida. Falta el token o el identificador." });
    } else {
      setUid(uid);
      setToken(tk);
    }
  ...
  const handlePasswordFormSubmit = async (e) => {
    ...
    if (!uidb64 || !token) {
      setMessage({ type: "danger", text: "❌ Error: No se pudo obtener la información de restablecimiento." });
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessage({ type: "danger", text: `❌ Error: ${data.message || "Error desconocido."}`});
      }
    } catch (error) {
      setMessage({type: "danger", text: `❌ Error: en la conexión: ${error.message}`});
    }
  };
      ...
      <div className="d-flex justify-content-center align-items-center flex-grow-1 mt-5 mb-5">
        <div className="card p-4 shadow-lg rounded-4" style={{ width: "400px", maxWidth: "85%", background: "linear-gradient(to right, #f4b183, #f8d8a8)"}}>
          ...
          <form onSubmit={handlePasswordFormSubmit}>
            ...
            {message.text && (
              <div className={`alert alert-${message.type} text-center mx-auto`} role="alert">
                {message.text}
              </div>
            )}
