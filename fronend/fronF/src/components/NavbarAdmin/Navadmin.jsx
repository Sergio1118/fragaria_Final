...
function Navbaradmin() {
  ...
  const processVoiceCommand = async (command) => {
    ...
    const empleadoMatch = normalizedCommand.match(/apolo cuántas (?:tareas|actividades) tiene (\w+\s\w+)/i) || 
    normalizedCommand.match(/cuántas (?:tareas|actividades) tiene (\w+\s\w+)/i) || 
    normalizedCommand.match(/cuáles son las (?:tareas|actividades) de (\w+\s\w+)/i);
    if (empleadoMatch) {
    ...
    if (
      ...
      navegarSiCoincide(normalizedCommand, ["plantación", "ve plantaciones"], "/plantacion") ||
      navegarSiCoincide(normalizedCommand, ["cronograma"], "/cronograma_admin") ||
      navegarSiCoincide(normalizedCommand, ["trabajador", "a trabajadores"], "/trabajador") ||
      navegarSiCoincide(normalizedCommand, ["informe", "a informes"], "/informes") ||
      navegarSiCoincide(normalizedCommand, ["actividad", "ve actividades", "ve a actividades"], "/actividad_admin")
    ) {
      setTimeout(() => setIsProcessing(false), 2000);
      return;
    }
